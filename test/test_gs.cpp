...
#define BOOST_TEST_MODULE GrayScottTest
...
#endif
// Test (0,1)
BOOST_AUTO_TEST_CASE(test_parameter_types)
{
    BOOST_CHECK((std::is_same<decltype(F), double>::value));
    BOOST_CHECK((std::is_same<decltype(k), double>::value));
...
BOOST_AUTO_TEST_CASE(test_same_size)
{
    ...
    for (size_t i = 0; i < u.size(); ++i) {
        BOOST_CHECK_EQUAL(u[i].size(), v[i].size());
    }
}

//Test (0,3)
BOOST_AUTO_TEST_CASE(test_elements_above_threshold)
{
    double proportion = countElementsAboveThreshold(0.1);
    BOOST_CHECK(proportion >= 0.0 && proportion <= 1.0);
}

//Test (0,4)
BOOST_AUTO_TEST_CASE(test_values_within_range)
{
    for (const auto& row : u) {
        for (const double& value : row) {
            BOOST_CHECK(value >= 0.0 && value <= 1.0);
        }
    }
